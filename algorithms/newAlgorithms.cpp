...
int main(){
  ...
  std::for_each_n(intVec.begin(), 5, [](int& arg){ arg *= arg; });
  ...
  std::string res = std::reduce(strVec2.begin() + 1, strVec2.end(), strVec2[0], 
                    [](auto fir, auto sec){ return fir + ":" + sec; });
  
  std::cout << "reduce: " << res << std::endl;
  
  /*std::size_t res7 = std::transform_reduce(strVec2.begin(), strVec2.end(), 
